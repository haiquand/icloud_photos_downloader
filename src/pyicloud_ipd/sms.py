...
from typing import Any, List, Mapping, NamedTuple, Protocol, Sequence, Tuple
...
class _SMSParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: List[Tuple[str, str | None]]) -> None:
        if tag == "script":
            self._is_boot_args = ("type", "application/json") in attrs and (
                "class",
                "boot_args",
            ) in attrs
...
def _map_to_trusted_device(device: Mapping[str, Any]) -> TrustedDevice | None:
    id: int | None = device.get("id")
    number: str | None = device.get("obfuscatedNumber")
...
def build_verify_sms_code_request(
    context: _TrustedPhoneContextProvider, device_id: int, code: str
) -> Request:
    """Builds a request for the list of trusted phone numbers for sms 2fa"""
